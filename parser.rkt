...
(define execute
  (lambda (search-query files)
    (if (string? search-query)
        (find-files-containing-word files search-query)

        (let ((operator (car search-query))
              (operand1 (list-ref search-query 1)))
          (let ((result1 (execute operand1 files)))
            (cond
              ((equal? operator "*")
               (if (null? result1)
                   '()
                   (let ((operand2 (list-ref search-query 2)))
                     (let ((result2 (execute operand2 files)))
...
(define find-files-containing-word
  (lambda (files word)
    (flatten
     (map
      (lambda (f)
        (let ((in (open-input-file (string->path f))))
          (if (string-contains? (read-string (file-size f) in) word)
              f
              '()
              )))
      files
...
(define minus
  (lambda (lst1 lst2)
    (if (null? lst1)
        ...
        (if (member (car lst1) lst2)
            (cdr lst1)
            (cons (car lst1) (minus (cdr lst1) lst2))
            )
        )
    )
  )
...
(define union
  (lambda (lst1 lst2)
    (minus (append lst1 lst2) (intersection lst1 lst2))
    )
  )
